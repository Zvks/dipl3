...
from tensorflow.keras.optimizers import Adam #Подключение оптимезатора Adam
...
model = keras.models.Sequential([
    keras.layers.Conv2D(filters=64, kernel_size=(3,3), strides=(1,1), activation='relu', input_shape=(IMG_HEIGHT, IMG_WIDTH, 1)),
...
store_learning = model.fit(x_data,  # ----------------- x_data, примеры набора данных
                           y_data,  # ----------------- y_data, метки примеров набора данных
                           ...
                           batch_size=10,  # ---------- размер пакета, который обрабатывает нейронка перед одним изменением весов
